...
object GraphXOps {
  implicit class GXOperations[VD: ClassTag, ED: ClassTag](graph: Graph[VD, ED]) {
    ...
    def scalableCommunityDetection(sc: SparkContext, logFilePath: String, queryTimeInterval: (Int, Int), queryTimeIntervalLength: Int): RDD[Row] = {
      ...
      val longEdges: RDD[Edge[(Int, Int)]] = graph.edges.map { e =>
        Edge(e.srcId.toInt, e.dstId.toInt, (e.attr.asInstanceOf[(Int, Int)]._1, e.attr.asInstanceOf[(Int, Int)]._2))
      }

      // Step 3: Create transformed Graph[Int, (Long, Long)]
      val transformedGraph: Graph[Int, (Int, Int)] = Graph(intVertexRDD, longEdges)
      DistributedWCC.run(transformedGraph, queryTimeInterval: (Int, Int), queryTimeIntervalLength, sc, logFilePath, isCanonical = true)
